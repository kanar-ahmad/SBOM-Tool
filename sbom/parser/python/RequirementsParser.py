...
from ...model.component import Component, HashType, Author
...
class RequirementsParser(BaseParser):
	def __init__(self, requirements_content: str, use_purl_bom_ref: bool = False, *,
		...
		for requirement in parsed_rf.requirements:
			...
			if not version and not requirement.is_local_path:
				self._warnings.append(
					ParserWarning(item=name, warning=(f"Requirement \'{name}\' does not have a pinned "
				...
				component = Component(name=name, bom_ref=bom_ref, version=version, hashes=hashes, purl=purl)

				from ...fetcher.Fetcher import Fetcher
				fetch_info = Fetcher().fetchPyPi(name=name, version=version)
				print(fetch_info)
				component.author = Author(name=fetch_info['information']['author'], email=fetch_info['information']['author_email'])
				#component.keywords += fetch_info['information']['keywords']
				#component.project_url = fetch_info['information']['project_url']
				print(component.author)
				print(component.keywords)
				print(component.project_url)
				print('********************')
				#exit(0)
