...
class _BaseCondaParser(BaseParser, metaclass=ABCMeta):
	...
	def __init__(self, conda_data: str, use_purl_bom_ref: bool = False, *,
	...
	def _conda_packages_to_components(self, use_purl_bom_ref: bool) -> None:
		...
		for conda_package in self._conda_packages:
			...
			c = Component(name=conda_package['name'], bom_ref=bom_ref, version=conda_package['version'], purl=purl)
			c.external_references.add(ExternalReference(reference_type=ExternalReferenceType.DISTRIBUTION,
				url=XsUri(conda_package['base_url']),
				comment=f"Distribution name {conda_package['dist_name']}"))
			if conda_package['md5_hash'] is not None:
				c.hashes.add(HashType(algorithm=HashAlgorithm.MD5, hash_value=str(conda_package['md5_hash'])))
