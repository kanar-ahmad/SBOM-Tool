...
class PipEnvParser(BaseParser):
	...
	def __init__(self, pipenv_contents: str, use_purl_bom_ref: bool = False, *,
		...
		for (package_name, package_data) in pipfile_default.items():
			...
			if isinstance(package_data.get('hashes'), list):
				...
				for pip_hash in package_data['hashes']:
					...
					ext_ref = ExternalReference(reference_type=ExternalReferenceType.DISTRIBUTION,
...
class PipEnvFileParser(PipEnvParser):
	...
	def __init__(self, pipenv_lock_filename: str, use_purl_bom_ref: bool = False, *,
			debug_message: DebugMessageCallback = quiet) -> None:
		debug_message('open file: {}', pipenv_lock_filename)
		with open(pipenv_lock_filename) as plf:
			super(PipEnvFileParser, self).__init__(pipenv_contents=plf.read(), use_purl_bom_ref=use_purl_bom_ref,
